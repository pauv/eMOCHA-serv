...
echo Html::anchor('stats_missing/export', 'Missing people').', ';
echo Html::anchor('stats_couples/export', 'Tested couples');
?>
</div>

<?php

if($selected_form_id=='missing') {
	
	$link = '/stats_missing/export_as_csv';
	echo Html::anchor($link, 'Download for spreadsheet import (tab separated)');
	
	echo $table; 
	
}
if($selected_form_id=='couples') {
	
	$link = '/stats_couples/export_as_csv';
	echo Html::anchor($link, 'Download for spreadsheet import (tab separated)');
